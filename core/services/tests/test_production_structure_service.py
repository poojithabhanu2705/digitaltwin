...
class ProductionStructureService:
    ...
    def create_station(self, **data):
        ...
        The line may be supplied either as:
        - a production line ID
        - a ProductionLine model instance
        """
        line = data.get("line")

        if line is not None:
            if hasattr(line, "line_id"):
                self.get_line(line.line_id)
            else:
                self.get_line(line)
    ...
    def create_equipment(self, **data):
        ...
        The station may be supplied either as:
        - a station ID
        - a Station model instance
        """
        station = data.get("station")

        if station is not None:
            if hasattr(station, "station_id"):
                self.get_station(station.station_id)
            else:
                self.get_station(station)
    ...
    def create_route(self, **data):
        ...
        The line and station may be supplied either as:
        - IDs
        ...
        if line is not None:
            if hasattr(line, "line_id"):
                line = self.get_line(line.line_id)
            else:
        ...
        if station is not None:
            if hasattr(station, "station_id"):
                station = self.get_station(station.station_id)
            else:
    ...
    def get_route_at_sequence(self, line_id, sequence_number):
        ...
        Raises:
            NotFoundError: If no route exists at that sequence.
        """
